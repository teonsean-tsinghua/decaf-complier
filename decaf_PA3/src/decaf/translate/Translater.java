...
public class Translater {
	...
	public void genCheckDivByZero(Temp divisor) {
		Temp zero = genLoadImm4(0);
		Temp cond = genEqu(divisor, zero);
		Label err = Label.createLabel();
		Label exit = Label.createLabel();
		genBnez(cond, err);
		genBranch(exit);
		genMark(err);
		Temp msg = genLoadStrConst(RuntimeError.DIVIDED_BY_ZERO_ERROR);
		genParm(msg);
	...
	public Temp genNewArray(Temp length, boolean isComplex) {
		...
		if(isComplex) {
			Temp csize = genLoadImm4(OffsetCounter.WORD_SIZE * 2);
			genParm(csize);
			Temp cptr = genIntrinsicCall(Intrinsic.ALLOCATE);
			genStore(zero, cptr, 0);
			genStore(zero, cptr, 4);
			genStore(cptr, obj, 0);
		} else {
			genStore(zero, obj, 0);
		}
	...
	public void genNewForClass(Class c) {
		...
		while(iter.hasNext()) {
			Symbol sym = iter.next();
			if(sym.isVariable()) {
				pos = genAdd(newObj, genLoadImm4(offset));
